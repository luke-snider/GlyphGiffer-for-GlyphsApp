...
class GlyphGiffer():
	...
	def __init__(self):
		...
		self.w.loadFontFolder = Button((10, 10, -10, 20), "Select fonts folder", callback=self.loadFontsFromFolder, sizeStyle="regular")
		...
		self.w.yShiftGlyph = EditText((110, y, 40, 20), 100, sizeStyle='small', callback=self.yShiftGlyphUserInput)
	...
	def walk(self, folder):
		...
		for n in names:
			...
			if file_extension[1:] in ["ufo","glyphs"]:
				files.append(p)
		return files
			...
			for thisFontPath in self.loadedFontList:
				...
				for index in range(int(str(self.w.speed.get()))):
					...
					if thisFont.glyphs[glyphName] is not None:
						glyph = thisFont.glyphs[glyphName].layers[selectionMaster]
						#RF
